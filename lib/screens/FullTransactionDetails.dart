...
class FullTransactionDetails extends StatefulWidget {
  List transactionDataList;
  String date;
  var milkman_id;

  FullTransactionDetails(this.milkman_id, this.transactionDataList, this.date);
  @override
  _FullTransactionDetailsState createState() => _FullTransactionDetailsState();
}

class _FullTransactionDetailsState extends State<FullTransactionDetails> {
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();
  var isLoading;
  var guestUserIndex;
  List finalList, userList;
  var updateUrl;
  var storage;
  var token;
  var productDropdown;
  ...
  void initState() {
    ...
    userList = [];
    pstDetails();
    getUserUrl = Constants.base_url + "users/get_users_owner";
    unit = "";
    print("final List" + finalList.toString());
    getProductsUrl = Constants.base_url + "products/get_all_products_owner";
    updateUrl = Constants.base_url + 'transactions/update_transaction_owner';
    getProducts();
    getUser();
  }
  ...
  Future<void> getUser() async {
    try {
      ...
      final storage = new FlutterSecureStorage();
      token = await storage.read(key: 'token');
      http.Response response = await http.post(getUserUrl, body: {
        'milkman_id': widget.milkman_id
      }, headers: {
        'Authorization': 'Bearer' + ' ' + token,
        "Accept": "application/json"
      ...
      if (response.statusCode == 200) {
        ...
        print("User Namesss" + userNames.toString());
      } else {}
    } catch (e) {
      checkInternetConnection();
    }
